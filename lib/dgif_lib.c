...
{
    //Buf长度为6  刚好覆盖GIF文件头(GIF87a或GIF89a)
    //GIFSTAMP_LEN 长度5 为GIF文件头去掉某位a的长度 这样可以判断是否GIF文件并比较最某位来确定GIF版本
    ...
    //通过判断文件最开始三个字节是否"GIF" 判断是否为GIF文件
    Buf[GIF_STAMP_LEN] = 0;
...
//读取判断GIF各个块的信息(Extension块(0x21),Descriptor块(0x2c),Terminator(文件末尾==0x3b))
...
DGifSetupDecompress(GifFileType *GifFile)
{
    ...
    //Warnning 以下cleanCode EOFCode以及具体图像读取的code不是以字节为单位 范围是2bits~12bits 以位为单位
    ...
    //code的位宽限制 从2bits-12bits 超出12bits后 会接收到一个cleanCode提醒须重设位宽
